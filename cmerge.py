...
def print_vars_per_host(varlist):
	...
	for host in host_list:  #References global host_list
		...
		for key in keylist:
			...
			#incorrect data
			pause = input("Press ENTER to Continue (q to QUIT)")
			if pause.lower() == "q":
...
def q_review_vars():
	...
	if args.verbose:
		...
		exit(0)
	while True:
		answ2 = input("Would you like to review the imported vars before generating configs? ")
...
def q_start_export():
	...
	while True:
		answ3 = input("\nWARNING: Exporting configurations will overwrite any files with the same name "\
...
def create_var_file():
	...
	if "<HOSTNAME>" in t_vars:
		...
		print("The template MUST contain a '<HOSTNAME>' variable (case sensitive).  Please add one after the "\
		+ "'hostname' or 'switchname' command in the template and try again.\n")
		exit(0)
	s_t_vars = sorted(t_vars)
	s_t_vars.insert(0,"<HOSTNAME>")

	with open(args.variables, 'w', newline='') as newfile:
		outputcsv = csv.writer(newfile)
		outputcsv.writerow(s_t_vars)
	print("Creation of variables file %s has completed successfully.\n" % args.variables)
...
def config_merge():
	...
	if not identical:
		while True:
			answ = input("Would you like to continue with the merge anyway? ")
	...
	if args.quiet:
		...
		while True:
			answ = input("Is this correct? (Type 'yes' to continue) ")
			if answ.lower() == "yes" or answ.lower() == "y":
				q_review_vars()
				break
			elif answ.lower() == "no" or answ.lower() == "n":
...
""" This section manages the (-h) help output, with the help of ArgParse, and makes sure that all 
	required arguments are passed to the script when run.
"""
parser = argparse.ArgumentParser(description="This script will generate a configuration file \
	for multiple devices.  It needs a 'template' configuration file that uses variable names \
	in every location where each device needs a different value.  This script also needs a \
	'variables' file that lists the variables and associated replacement value for the device.  \
	This variables file can contain sections for multiple configuration files.")
parser.add_argument("-c", "--create_vars", action="store_true", help="Create a variables file \
	containing all vars found in the template.  The data will be written to the file specified \
	in the 'variables' argument")
group = parser.add_mutually_exclusive_group()
group.add_argument("-v", "--verbose", action="store_true", help="Will provide a more verbose \
	output and automatically trigger a review of all parsed variables")
group.add_argument("-q", "--quiet", action="store_true", help="Will provide minimal output and \
	will automatically skip all review questions")
...
try:
	...
	print("ERROR: The template file %s cannot be found, please try again with a valid filename\n" % args.template)
	exit(1)
...
if args.create_vars and os.path.isfile(args.variables):	
	...
	print("If you are trying to generate config files, please remove the '-c' argument.")
	print("If you are trying to create a variables file, please supply an unused filename\n")
	...
	try:
		...
		print("ERROR: The variables file %s cannot be found, please try again with a valid filename\n" % args.variables)
		exit(1)
